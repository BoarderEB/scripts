...
# Backup starten
if [[ $GPG == "true" ]]; then
    LOGGERMASSAGE "Start tar and gpg encryption of $SOURCE_DIR"
    tar -czf - -C "$(dirname "$SOURCE_DIR")" "$(basename "$SOURCE_DIR")" | gpg2 --encrypt -a --recipient $GPGID --trust-model always > "$BACKUPPATH/www-backup-$DATE.tar.gz.gpg"
    if [[ ${PIPESTATUS[0]} != 0 ]]; then
        EXPORTERROR="true"
        LOGGERMASSAGE 0 "Error: there was a problem compressing or encrypting $SOURCE_DIR"
    else
        LOGGERMASSAGE "Backup and encryption of $SOURCE_DIR successfully completed"
    fi
else
    LOGGERMASSAGE "Start tar of $SOURCE_DIR"
    tar -czf "$BACKUPPATH/www-backup-$DATE.tar.gz" -C "$(dirname "$SOURCE_DIR")" "$(basename "$SOURCE_DIR")"
    if [[ $? -ne 0 ]]; then
        EXPORTERROR="true"
        LOGGERMASSAGE 0 "Error: there was a problem compressing $SOURCE_DIR"
    else
        LOGGERMASSAGE "Backup of $SOURCE_DIR successfully completed"
    fi
fi
